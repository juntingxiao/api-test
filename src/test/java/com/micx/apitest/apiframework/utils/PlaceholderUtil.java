...
public class PlaceholderUtil {
    ...
    public String resolveByRule(String content, Map<String, Object> actionParams) {
        int start = content.indexOf(this.placeholderPrefix);
        if (start == -1) {
            return content;
        }
        ...
        while (start != -1) {
            ...
            if(StringUtils.isEmpty(placeholder.trim())){
                return result.toString();
            }
            if(actionParams.containsKey(placeholder)) {
                replaceContent = actionParams.get(placeholder).toString();
            } else {
                return result.toString();
            }
            result.replace(start, end + this.placeholderSuffix.length(), replaceContent);
     ...
     * @return
     */
    public String resolveByMapReturnAsStr(String content, final Map<String, Object> valueMap) {
        return resolveByRule(content, valueMap);
    }
     ...
     * @return
     */
    public List<Object> resolve(List<Object> content, final Map<String, Object> valueMap) {
        List<Object> result = new ArrayList<>();
        content.forEach(x -> {
            result.add(resolveByRule(x.toString(), placeholderValue -> String.valueOf(valueMap.get(placeholderValue))));
        });
        return result;
    }
...
//        action.put("name1", "soso");
//        action.put("name", "曹测试");

//        params.forEach(x -> {
//            List<Object> s = PlaceholderUtil.getDefaultResolver().resolve(x.toString(), action);
//            result.addAll(s);
//        });
//        result = PlaceholderUtil.getDefaultResolver().resolve(params, action);
//        System.out.println(result);
//        Map<String, Object> actionParams = new HashMap<>();
//        actionParams.put("ossFileName1", "soso");
//        actionParams.put("workOrderId", "sldfjlskjdflsjdlf");
//
//        Map<String, Object> param = new HashMap<>();
//        param.put("ossFileName","${ossFileName1}");
//
//        JSONObject jsonObject = new JSONObject();
//        jsonObject.put("ossFileName",param);
//        jsonObject.put("workOrderId","${workOrderId}");
//
//        String body = jsonObject.toJSONString();
//        String resolver_result = PlaceholderUtil.getDefaultResolver()
//                .resolveByMap(body, actionParams);
//        System.out.println(resolver_result);

//        String content ="\"Authorization\": \"Basic ${base64}\"";
        String content ="{\n" +
                "  \"ossFileName\": {\n" +
                "    \"ossFileName\": \"11\"\n" +
                "  },\n" +
        ...
        Map<String, Object> param = new HashMap<>();
        param.put("name","soso");
        param.put("base64","胜多负少的发生的");

        String resolveValue = getDefaultResolver().resolveByRule(content, param);
